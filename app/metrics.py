...
def get_graphic_product_category_metric():
    ...
    products = Product.objects.all()
    return {category.name: products.filter(category=category).count() for category in categories}


def get_graphic_product_brand_metric():
    brands = Brand.objects.all()
    products = Product.objects.all()
    return {brand.name: products.filter(brand=brand).count() for brand in brands}
